...
public class ColorPhrase {
	 ...
	 * @return
	 */
	public ColorPhrase innerSize(int innerSize) {
	 ...
	 * @return
	 */
	public ColorPhrase outSize(int outerSize) {
		this.outerSize = outerSize;
		return this;
	}
	
	/**
	 * init innerClickListener
	 *
	 * @param innerClickListener 内部文字点击事件
	 * @return
	 */
	public ColorPhrase innerClickListener(View.OnClickListener innerClickListener) {
	...
	private InnerToken inner(Token prev) {
		...
		return new InnerToken(prev, key, innerColor, innerSize, innerClickListener);
	}
	...
	private static class OuterToken extends Token {
		...
		OuterToken(Token prev, int textLength, int _color, int _size) {
			super(prev);
		...
		void expand(SpannableStringBuilder target) {
			...
			if (size >= 12) {
				target.setSpan(new AbsoluteSizeSpan(size), startPoint, endPoint, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (size > 0 && size < 12) {
				throw new IllegalArgumentException("outerSize must greater than 12");
			}
		}
	...
	private static class InnerToken extends Token {
		...
		InnerToken(Token prev, String _inner, int _color, int _size, View.OnClickListener listener) {
			super(prev);
		...
		void expand(SpannableStringBuilder target) {
			...
			if (size >= 12) {
				target.setSpan(new AbsoluteSizeSpan(size), replaceFrom, replaceTo - 2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			} else if (size > 0 && size < 12) {
				throw new IllegalArgumentException("innerSize must greater than 12");
			}
			if (this.clickListener != null) {
				target.setSpan(new OnSpanClick(clickListener, color), replaceFrom, replaceTo - 2, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
		}
	...
	private static class OnSpanClick extends ClickableSpan {
		private final View.OnClickListener mListener;
		private int color;
		
		OnSpanClick(View.OnClickListener l, int innerColor) {
			mListener = l;
