...
import com.GuoGuo.JuicyChat.ui.widget.ReadReceiptViewPager;
...
public class ReadReceiptDetailActivity extends BaseActivity {
					...
					while (iter.hasNext()) {
						...
						if (getMemberIndex(key.toString(), groupMember) == -1) { //不在群组中
							...
							if (userInfo != null) {
								...
								readMember.setRemark(userInfo.getName());
							} else {
								readMember = new GroupMember();
								readMember.setRemark(key.toString());
							}
							if (mReadMember != null) {
	...
	private void initViews() {
		...
		if (mMessage != null) {
			...
			if (friendInfo != null) {
				String displayName = friendInfo.getUsername();
				if (!TextUtils.isEmpty(displayName)) {
					tv_name.setText(displayName);
				} else {
					tv_name.setText(friendInfo.getUsername());
				}
			} else {
...
//            String portraitUri = SealUserInfoManager.getInstance().getPortraitUri(bean);
			...
			iv_avatar.setOnClickListener(new View.OnClickListener() {
				@Override
				public void onClick(View v) {
					UserInfo userInfo = new UserInfo(bean.getUserid(), bean.getNickname(), TextUtils.isEmpty(bean.getUserhead()) ? Uri.parse(RongGenerate.generateDefaultAvatar(bean.getNickname(), bean.getUserid())) : Uri.parse(bean.getUserhead()));
	...
	private Friend getUserInfoById(String userId) {
		if (!TextUtils.isEmpty(userId)) {
			return DBManager.getInstance().getDaoSession().getFriendDao().queryBuilder().where(FriendDao.Properties.Myid.eq(userId)).unique();
		}
		return null;
	}
