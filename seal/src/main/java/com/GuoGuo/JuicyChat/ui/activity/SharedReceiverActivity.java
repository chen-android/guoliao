...
import com.GuoGuo.JuicyChat.ui.widget.linkpreview.LinkPreviewCallback;
import com.GuoGuo.JuicyChat.ui.widget.linkpreview.SourceContent;
import com.GuoGuo.JuicyChat.ui.widget.linkpreview.TextCrawler;
				...
				for (Conversation c : conversationsList) {
					if (c.getConversationType().equals(Conversation.ConversationType.GROUP)) {
						for (Groups group : mGroupData) {
							if (group.getGroupid().equals(c.getTargetId())) {
								tempList.add(c);
							}
						}
				...
				if (tempList.size() > 0) {
					for (Conversation conversation : tempList) {
						if (conversation.getConversationType().equals(Conversation.ConversationType.PRIVATE)) {//TODO 会话中包含自己本身会 crash
							...
							if (friend == null)
								continue;
							if (friend != null && !TextUtils.isEmpty(friend.getHeadico().toString())) {
								portraitUri = friend.getHeadico().toString();
							} else {
								if (friend != null) {
									portraitUri = RongGenerate.generateDefaultAvatar(friend.getUsername(), friend.getFriendid());
								}
							...
							newConversationsList.add(
									new NewConversation(Conversation.ConversationType.PRIVATE,
											...
											portraitUri,
											friend == null ? null : friend.getUsername()));
						} else {
							Groups groups = getGroupInfoById(conversation.getTargetId());
							String portraitUri = null;
							if (groups == null)
								continue;
							if (groups != null && !TextUtils.isEmpty(groups.getGrouphead())) {
								portraitUri = groups.getGrouphead();
							} else {
								if (groups != null) {
									portraitUri = RongGenerate.generateDefaultAvatar(groups.getGroupname(), groups.getGroupid());
								}
							}
							newConversationsList.add(
									new NewConversation(Conversation.ConversationType.GROUP,
											conversation.getTargetId(),
											portraitUri,
											groups == null ? null : groups.getGroupname()));
						}
					}
