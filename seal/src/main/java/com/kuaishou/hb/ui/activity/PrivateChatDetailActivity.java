	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.ac_ll_search_messages:
				...
				Friend friend = DBManager.getInstance().getDaoSession().getFriendDao().queryBuilder().where(FriendDao.Properties.Id.eq(fromConversationId)).unique();
				...
				if (friend != null) {
					String portraitUri = friend.getHeadico().toString();
					mResult.setId(friend.getMyid());
					if (!TextUtils.isEmpty(portraitUri)) {
						mResult.setPortraitUri(portraitUri);
					}
					if (!TextUtils.isEmpty(friend.getUsername())) {
						mResult.setTitle(friend.getUsername());
					} else {
						mResult.setTitle(friend.getUsername());
					}
