...
import com.kuaishou.hb.server.utils.OperationRong;
...
import com.kuaishou.hb.ui.widget.DemoGridView;
import com.kuaishou.hb.ui.widget.switchbutton.SwitchButton;
...
public class DiscussionDetailActivity extends BaseActivity implements CompoundButton.OnCheckedChangeListener, View.OnClickListener {
			...
			if (position == getCount() - 1 && isCreated) {
				...
				iv_avatar.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						startActivityForResult(intent, GGConst.DISCUSSION_REMOVE_MEMBER_REQUEST_CODE);
					}
				...
				iv_avatar.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						startActivityForResult(intent, GGConst.DISCUSSION_ADD_MEMBER_REQUEST_CODE);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			switch (requestCode) {
				case GGConst.DISCUSSION_ADD_MEMBER_REQUEST_CODE:
									...
									if (friendList != null && friendList.size() > 0) {
										for (Friend friend : friendList) {
											for (String userId : addMember) {
												if (userId.equals(friend.getFriendid()))
													memberList.add(new UserInfo(userId, friend.getUsername(), Uri.parse(friend.getHeadico())));
											}
										}
					...
					break;
				case GGConst.DISCUSSION_REMOVE_MEMBER_REQUEST_CODE:
		...
		switch (requestCode) {
			case FIND_USER_INFO:
				...
				if (response.getCode() == 200) {
					...
					String loginId = getSharedPreferences("config", MODE_PRIVATE).getString(GGConst.GUOGUO_LOGIN_ID, "");
