...
import com.kuaishou.hb.model.SearchResult;
import com.kuaishou.hb.ui.adapter.GroupListAdapter;
...
public class SealSearchMoreGroupActivity extends Activity {
		...
		mGroupsListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (selectObject instanceof String) {
					...
					if (groupInfo != null) {
						RongIM.getInstance().startGroupChat(SealSearchMoreGroupActivity.this, groupInfo.getGroupid(), groupInfo.getGroupname());
					}
				}
			}
		});
		...
		for (String groupId : mFilterGroupId) {
			...
			List<GroupMember> filterGroupNameList = groupNameQueryBuilder.where(GroupMemberDao.Properties.Groupid.eq(groupId),
					groupNameQueryBuilder.or(GroupMemberDao.Properties.Groupname.like("%" + mFilterString + "%"),
							GroupMemberDao.Properties.Groupid.like(mFilterString + "%"))).orderAsc(GroupMemberDao.Properties.Groupid).build().list();
			...
			List<GroupMember> filterGroupMemberNameList = groupMemberNameQueryBuilder.where(GroupMemberDao.Properties.Groupid.eq(groupId),
					groupMemberNameQueryBuilder.or(GroupMemberDao.Properties.Username.like("%" + mFilterString + "%"),
							GroupMemberDao.Properties.Nickname.like(mFilterString + "%"),
							GroupMemberDao.Properties.Remark.like("%" + mFilterString + "%")))
					.orderAsc(GroupMemberDao.Properties.Userid, GroupMemberDao.Properties.Username).build().list();
	...
	private synchronized SearchResult filterInfo(String filterStr) {
		...
		Cursor cursor = DBManager.getInstance().getDaoSession().getDatabase().rawQuery(SQL_DISTINCT_GROUP_ID
				...
				+ GroupMemberDao.Properties.Groupid.columnName + " like " + "'" + filterStr + "%" + "'" + " or "
//                        + GroupMemberDao.Properties.Name.columnName + " like " + "'" + "%" + filterStr + "%" + "'" + " or "
				+ GroupMemberDao.Properties.Username.columnName + " like " + "'" + filterStr + "%" + "'" + " or "
				+ GroupMemberDao.Properties.Nickname.columnName + " like " + "'" + "%" + filterStr + "%" + "'" + " or "
				+ GroupMemberDao.Properties.Remark.columnName + " like " + "'" + filterStr + "%" + "'", null);
		...
		for (String groupId : filterGroupId) {
			...
			List<GroupMember> filterGroupNameList = groupNameQueryBuilder.where(GroupMemberDao.Properties.Groupid.eq(groupId),
					groupNameQueryBuilder.or(GroupMemberDao.Properties.Groupname.like("%" + filterStr + "%"),
							GroupMemberDao.Properties.Groupid.like(filterStr + "%"))).orderAsc(GroupMemberDao.Properties.Groupid).build().list();
			QueryBuilder groupMemberNameQueryBuilder = DBManager.getInstance().getDaoSession().getGroupMemberDao().queryBuilder();
			List<GroupMember> filterGroupMemberNameList = groupMemberNameQueryBuilder.where(GroupMemberDao.Properties.Groupid.eq(groupId),
