package com.kuaishou.hb.server.pinyin;
...
public class CharacterParser {
	...
	public SpannableStringBuilder getColoredNameList(String filterStr, List<GroupMember> filterGroupMemberList) {
		...
		for (GroupMember groupMember : filterGroupMemberList) {
			if (!TextUtils.isEmpty(groupMember.getNickname())) {
				SpannableStringBuilder spannableStringBuilder = getColored(filterStr, groupMember.getNickname());
	...
	public Friend generateFriendFromUserInfo(UserInfo userInfo) {
		...
		if (userInfo != null) {
			return new Friend(userInfo.getUserId(), userInfo.getName(), userInfo.getPortraitUri().toString());
		}
		return null;
	}
	...
	public UserInfo generateUserInfoFromFriend(Friend friend) {
		if (friend != null) {
			UserInfo userInfo = new UserInfo(friend.getMyid(),
					friend.getUsername(),
					Uri.parse(friend.getHeadico()));
			return userInfo;
		}
